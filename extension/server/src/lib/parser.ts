...
export async function isSymbolDeclared(
  ...
  const files = scopedFilePath ? [scopedFilePath] : await getAllSourceFiles(workspaceRoot);

  for (const filePath of files) {
    let content: string;
    try {
      content = fileContentOverride ?? (await fs.promises.readFile(filePath, 'utf8'));
    } catch (e) {
 ...
 */
export function getWordAtPosition(lineText: string, character: number): WordInfo | undefined {
...
export const CODE_KEYWORDS = [
  ...
  'GLOBAL', 'DEF', 'DEFFCT', 'END', 'ENDFCT', 'RETURN',
  'DEFDAT', 'ENDDAT', 'PUBLIC', 'CONST',
  // Kontrol yapıları
  'IF', 'THEN', 'ELSE', 'ENDIF',
  'FOR', 'TO', 'STEP', 'ENDFOR',
  'WHILE', 'ENDWHILE',
  ...
  'TRUE', 'FALSE',
  // Bit operatörleri
  'B_AND', 'B_OR', 'B_NOT', 'B_XOR', 'B_NAND', 'B_NOR', 'B_XNOR',
  // Veri tipleri
  'DECL', 'INT', 'REAL', 'BOOL', 'CHAR', 'STRING',
  'FRAME', 'POS', 'E6POS', 'AXIS', 'E6AXIS', 'LOAD',
  'STRUC', 'ENUM', 'SIGNAL',
  // Hareket komutları
  'PTP', 'LIN', 'CIRC', 'SPTP', 'SLIN', 'SCIRC',
  'PTP_SPLINE', 'LIN_SPLINE', 'CP_SPLINE',
  'C_PTP', 'C_LIN', 'C_VEL', 'C_DIS',
  // SPLINE parametreleri (SPTP, SLIN, SCIRC için)
  'SVEL', 'SVEL_JOINT', 'SACC', 'SACC_JOINT',
  'STOOL', 'STOOL2', 'SBASE', 'SLOAD',
  'SIPO_MODE', 'SAPO', 'SAPO_PTP', 'SAPO_LIN',
  'SGEAR_JERK', 'STIME', 'SCIRCTYPE',
  'SORI', 'SORI_EX', 'SJERK',
  'USE_CM_PRO_VALUES',
  // Kesme ve kontrol
  'INTERRUPT', 'TRIGGER', 'BRAKE', 'RESUME',
  'HALT', 'STOP', 'EXIT', 'GOTO', 'CONTINUE', 'BREAK',
  'WAIT', 'SEC', 'DELAY',
  // IR (Interrupt) sabitleri
  'IR_STOPM', 'IR_STOPMESS',
  // Fonksiyonlar
  'BAS', 'BAS_COMMAND', 'EXT', 'DMY',
  'SWRITE', 'SREAD', 'CWRITE', 'CREAD',
  'CAST_TO', 'CAST_FROM',
  'ON_ERROR_PROCEED', 'ERR_CLEAR', 'ERR_RAISE',
  'MBX_REC', 'VARSTATE', 'PULSE',
  // Mesaj fonksiyonları
  'MsgNotify', 'MsgQuit', 'MsgDialog', 'MsgConfirm',
  'SET_KRLMSG', 'EXISTS_KRLMSG', 'CLEAR_KRLMSG',
  // Matematik fonksiyonları
  'ABS', 'SIN', 'COS', 'TAN', 'ASIN', 'ACOS', 'ATAN2',
  'SQRT', 'MAX', 'MIN',
  // Kinematik
  'INVERSE', 'FORWARD', 'BASE', 'TOOL', 'LOAD_DATA',
  'NULLFRAME',
  // Koordinatlar
  'X', 'Y', 'Z', 'A', 'B', 'C', 'S', 'T',
  'A1', 'A2', 'A3', 'A4', 'A5', 'A6',
  'E1', 'E2', 'E3', 'E4', 'E5', 'E6',
  // Tetikleme
  'WHEN', 'DO', 'DISTANCE', 'PRIO', 'ON', 'WITH', 'IN', 'OUT',
  // Hareket parametreleri
  'PTP_PARAMS', 'CP_PARAMS', 'APO', 'VEL', 'ACC', 'OV_PRO',
  // Sistem modları
  'T1', 'T2', 'AUT', 'MANUAL', 'TCP',
  // MASREF ve sistem değişkenleri
  'R_GAP_BETWEEN', 'R_TABLE_LENGTH', 'MASREFG_GROUPSEQUENCE',
