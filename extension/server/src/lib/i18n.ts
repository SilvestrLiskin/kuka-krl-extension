...
const en: ServerMessages = {
    ...
    'diag.nonAsciiChar': 'Non-ASCII character "{0}" may cause errors in KUKA controller.',
    'diag.unclosedString': 'Unclosed string literal.',
    'diag.nameTooLong': 'Name "{0}" exceeds KUKA 24-character limit ({1} characters).',
    ...
    'diag.velocityTooHigh': 'Velocity {0} m/s exceeds maximum KUKA limit (3 m/s).',
    'diag.ptpVelocityTooHigh': 'PTP velocity {0}% exceeds maximum allowed (100%).',
    'diag.toolNotInitialized': 'Movement without $TOOL initialization. Use BAS(#INITMOV) or set $TOOL first.',
    'diag.baseNotInitialized': 'Movement without $BASE initialization. Use BAS(#INITMOV) or set $BASE first.',
    ...
    'diag.waitWithoutTimeout': 'WAIT FOR without timeout may cause indefinite wait.',
    'diag.dangerousHalt': 'HALT stops program execution. Use with caution.',

    'diag.realInSwitch': 'REAL type cannot be used in SWITCH/CASE. Use INT or ENUM.',
    'diag.typeMismatch': 'Type mismatch: assigning {0} to {1} variable "{2}".',
    'diag.shouldBeReal': 'Value {0} contains decimal. Variable "{1}" should be REAL, not INT.',
...
const ru: ServerMessages = {
    ...
    'diag.nonAsciiChar': 'Не-ASCII символ "{0}" может вызвать ошибки в контроллере KUKA.',
    'diag.unclosedString': 'Незакрытая строка.',
    'diag.nameTooLong': 'Имя "{0}" превышает лимит KUKA в 24 символа ({1} символов).',
    ...
    'diag.toolNotInitialized': 'Движение без инициализации $TOOL. Используйте BAS(#INITMOV) или установите $TOOL.',
    'diag.baseNotInitialized': 'Движение без инициализации $BASE. Используйте BAS(#INITMOV) или установите $BASE.',

    'diag.unmatchedBlock': 'Незакрытый блок "{0}" — отсутствует "{1}".',
    'diag.duplicateName': 'Дублирование {0} "{1}" (впервые определено в строке {2}).',
    'diag.deadCode': 'Недостижимый код после "{0}".',
    'diag.emptyBlock': 'Пустой блок "{0}".',
    'diag.waitWithoutTimeout': 'WAIT FOR без таймаута может вызвать бесконечное ожидание.',
    'diag.dangerousHalt': 'HALT останавливает выполнение программы. Используйте осторожно.',

    'diag.realInSwitch': 'Тип REAL нельзя использовать в SWITCH/CASE. Используйте INT или ENUM.',
    'diag.typeMismatch': 'Несовпадение типов: присваивание {0} переменной {1} "{2}".',
    'diag.shouldBeReal': 'Значение {0} содержит десятичную часть. Переменная "{1}" должна быть REAL, а не INT.',
...
const tr: ServerMessages = {
    ...
    'diag.nonAsciiChar': 'ASCII olmayan karakter "{0}" KUKA kontrolcüsünde hata oluşturabilir.',
    'diag.unclosedString': 'Kapatılmamış string.',
    'diag.nameTooLong': '"{0}" adı KUKA 24 karakter sınırını aşıyor ({1} karakter).',
    ...
    'diag.toolNotInitialized': '$TOOL başlatılmadan hareket. BAS(#INITMOV) kullanın veya önce $TOOL ayarlayın.',
    'diag.baseNotInitialized': '$BASE başlatılmadan hareket. BAS(#INITMOV) kullanın veya önce $BASE ayarlayın.',
    ...
    'diag.waitWithoutTimeout': 'Zaman aşımı olmadan WAIT FOR sonsuz beklemeye neden olabilir.',
    'diag.dangerousHalt': 'HALT program yürütmesini durdurur. Dikkatli kullanın.',

    'diag.realInSwitch': 'REAL tipi SWITCH/CASE içinde kullanılamaz. INT veya ENUM kullanın.',
    'diag.typeMismatch': 'Tip uyuşmazlığı: {0} değeri {1} tipindeki "{2}" değişkenine atanıyor.',
    'diag.shouldBeReal': '{0} değeri ondalık içeriyor. "{1}" değişkeni INT değil REAL olmalı.',
 ...
 */
export function t(key: keyof ServerMessages, ...args: (string | number)[]): string {
 ...
 */
export function matchesDiagnosticPattern(message: string, pattern: 'variableNotDefined' | 'globalPublicMismatch'): boolean {
    switch (pattern) {
        ...
        case 'globalPublicMismatch':
            return (message.includes('GLOBAL') && message.includes('PUBLIC')) ||
