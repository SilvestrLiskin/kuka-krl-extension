...
const OPEN_BLOCK_REGEXES: Map<string, RegExp> = new Map();
for (const key of Object.keys(BLOCK_PAIRS)) {
  OPEN_BLOCK_REGEXES.set(key, new RegExp(`\\b${key}\\b`, "i"));
}

const CLOSE_BLOCK_REGEXES: Map<string, RegExp> = new Map();
for (const key of Object.keys(CLOSE_TO_OPEN)) {
  CLOSE_BLOCK_REGEXES.set(key, new RegExp(`\\b${key}\\b`, "i"));
}
