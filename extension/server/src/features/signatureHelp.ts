...
export class SignatureHelpProvider {
    ...
    public onSignatureHelp(
        ...
        // Fonksiyon adını bul
        const textBeforeParen = textBeforeCursor.substring(0, openParenIndex).trim();
        ...
        const funcType = funcInfo.name.toLowerCase().includes('fct') ? 'function' : 'subroutine';
        const signature: SignatureInformation = {
            label: `${funcInfo.name}(${funcInfo.params})`,
            documentation: t('signature.userDefined', funcType),
            parameters: params_list.map((p) => ({
                label: p.trim(),
