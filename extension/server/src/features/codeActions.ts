...
import { t, matchesDiagnosticPattern, extractVariableFromMessage } from '../lib/i18n';
...
export class CodeActionsProvider {
    ...
    public onCodeAction(
        ...
        for (const diagnostic of diagnostics) {
            ...
            if (matchesDiagnosticPattern(diagnostic.message, 'variableNotDefined')) {
                ...
                if (varName) {
                    // INT olarak tanımla
                    actions.push(this.createDeclareVariableAction(doc, diagnostic, varName, 'INT'));
                    // REAL olarak tanımla
                    actions.push(this.createDeclareVariableAction(doc, diagnostic, varName, 'REAL'));
                    // BOOL olarak tanımla
                    actions.push(this.createDeclareVariableAction(doc, diagnostic, varName, 'BOOL'));
                }
            }

            // GLOBAL without PUBLIC warning
            if (matchesDiagnosticPattern(diagnostic.message, 'globalPublicMismatch')) {
                const lines = doc.getText().split(/\r?\n/);
                const line = lines[diagnostic.range.start.line];

                if (diagnostic.message.includes('not PUBLIC') ||
                ...
                } else if (diagnostic.message.includes('not GLOBAL') ||
            ...
            if (diagnostic.code === 'realInSwitch') {
                actions.push(...this.createChangeTypeActions(doc, diagnostic, 'REAL', 'INT'));
            }

            // Quick Fix для дробного числа в INT — изменить тип на REAL или обернуть в ROUND()
            if (diagnostic.code === 'shouldBeReal') {
                actions.push(...this.createChangeTypeActions(doc, diagnostic, 'INT', 'REAL'));
    ...
    private createDeclareVariableAction(
        ...
        return {
            ...
            edit: {
                changes: {
                    [doc.uri]: [
                        TextEdit.insert(Position.create(insertLine, 0), newDecl)
                    ]
                }
            }
        };
    ...
    private createChangeTypeActions(
        ...
        const actions: CodeAction[] = [];
        const data = diagnostic.data as { varName?: string; line?: number } | undefined;
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (match) {
                ...
                const title = toType === 'INT' ? t('action.changeToInt') : t('action.changeToReal');
    ...
    private createWrapWithRoundAction(
        ...
        const data = diagnostic.data as { varName?: string; value?: string; line?: number } | undefined;
        const lines = doc.getText().split(/\r?\n/);
        const lineIndex = diagnostic.range.start.line;
        const line = lines[lineIndex] || '';

        // Находим присваивание и оборачиваем значение в ROUND()
        const assignRegex = new RegExp(`(${data?.varName}\\s*=\\s*)(${data?.value?.replace('.', '\\.')})`, 'i');
