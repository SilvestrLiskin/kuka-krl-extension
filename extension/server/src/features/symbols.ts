...
export class DocumentSymbolsProvider {
    ...
    public onDocumentSymbols(
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const defMatch = line.match(/^\s*(?:GLOBAL\s+)?DEF\s+(\w+)\s*\(([^)]*)\)/i);
            if (defMatch) {
            ...
            const deffctMatch = line.match(/^\s*(?:GLOBAL\s+)?DEFFCT\s+(\w+)\s+(\w+)\s*\(([^)]*)\)/i);
            if (deffctMatch) {
            ...
            for (let j = symbolStack.length - 1; j >= 0; j--) {
                if (symbolStack[j].endPattern.test(line)) {
                    ...
                    if (symbolStack.length > 0) {
                        ...
                        symbolStack[symbolStack.length - 1].symbol.children!.push(completed.symbol);
                    } else {
                        // Top-level symbol
            ...
            const declMatch = line.match(/^\s*(?:GLOBAL\s+)?(?:DECL\s+)?(\w+)\s+(\w+)/i);
            if (declMatch && !defMatch && !deffctMatch && !defdatMatch) {
                ...
                const keywords = ['DEF', 'DEFFCT', 'DEFDAT', 'END', 'ENDFCT', 'ENDDAT', 'IF', 'FOR', 'WHILE', 'LOOP', 'SWITCH'];
                if (keywords.includes(typeName)) continue;

                // Only show DECL statements or type-prefixed declarations
                const isDecl = /^\s*(?:GLOBAL\s+)?DECL\b/i.test(line) ||
                    /^\s*(?:GLOBAL\s+)?(INT|REAL|BOOL|CHAR|STRING|FRAME|POS|E6POS|AXIS|E6AXIS|LOAD|SIGNAL)\s+\w+/i.test(line);
            ...
            if (signalMatch) {
                ...
                if (symbolStack.length > 0) {
                    symbolStack[symbolStack.length - 1].symbol.children!.push(signalSymbol);
                } else {
            ...
            if (strucMatch) {
                ...
                if (symbolStack.length > 0) {
                    symbolStack[symbolStack.length - 1].symbol.children!.push(strucSymbol);
                } else {
        ...
        while (symbolStack.length > 0) {
            ...
            if (symbolStack.length > 0) {
                symbolStack[symbolStack.length - 1].symbol.children!.push(unclosed.symbol);
            } else {
