 ...
 */
export async function findSystemVariables(): Promise<Map<string, vscode.Location[]>> {
    ...
    const sysVarPattern = /\$[A-Za-z_][A-Za-z0-9_]*(?:\.[A-Za-z_][A-Za-z0-9_]*)?(?:\[\d+\])?/g;
...
export async function showSystemVariablesPicker(): Promise<void> {
    const statusBar = vscode.window.setStatusBarMessage('$(sync~spin) Scanning for system variables...');
    ...
    try {
        ...
        const sortedVars = [...sysVars.entries()].sort((a, b) => a[0].localeCompare(b[0]));
