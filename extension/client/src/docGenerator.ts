...
export class DocGenerator {
    ...
    public async generateDocumentation(): Promise<void> {
        ...
        if (!editor) {
            vscode.window.showWarningMessage('Нет открытого файла для генерации документации.');
            return;
        }
        ...
        docContent.push('');
        docContent.push(`> Auto-generated from \`${path.basename(document.fileName)}\``);
        docContent.push('');
        docContent.push('---');
        docContent.push('');
        ...
        if (functions.length > 0) {
            ...
            for (const func of functions) {
                docContent.push(`| \`${func.name}\` | ${func.type} | ${func.params || '-'} | ${func.line + 1} |`);
            }
            docContent.push('');
        ...
        vscode.window.showInformationMessage(`Документация создана: ${path.basename(docPath)}`);
    }
    ...
    private extractFunctions(lines: string[]): Array<{
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (match) {
                const type = match[1].toUpperCase() === 'DEFFCT' ? 'Function' : 'Module';
